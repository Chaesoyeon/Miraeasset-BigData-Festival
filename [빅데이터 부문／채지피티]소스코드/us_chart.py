...
#클라우드에서 미국 주가데이터 불러오기
def uschart_download_stock_data(s3, bucket_name, ticker):
...
def uschart_process_dataframe(df):
    selected_rows = []
    ...
    for index in range(1, len(df)):
        ...
        if row['Change'] >= 0:  # 양수에서 음수로 바뀐 경우
            if consecutive_count_up == 0:
                start_date_up = prev_row['Date']  # 양수에서 음수로 바뀐 날짜 기록
            ...
            if consecutive_count_up > 0:
                ...
                selected_rows.append({
                    ...
                    'End Date': prev_row['Date'],  # 이전 row의 날짜로 설정
        ...
        if row['Change'] <= 0:  # 음수에서 양수로 바뀐 경우
            if consecutive_count_down == 0:
                start_date_down = prev_row['Date']  # 음수에서 양수로 바뀐 날짜 기록
    ...
    if selected_rows:
        ...
        return pd.DataFrame()  # 빈 데이터프레임 출력


#클라우드에 업로드
def uschart_upload(ticker, data, s3, bucket_name):
...
def uschart_main():
    ...
    for index, row in us_company_list.iterrows():
        ...
        st_data = uschart_download_stock_data(s3, bucket_name, ticker)
        st_data = uschart_six_month(st_data)

        result_df = uschart_process_dataframe(st_data)
        result_df['Ticker'] = ticker

        # 주가 데이터 수집 및 파일로 저장
        uschart_upload(ticker, result_df, s3, bucket_name)