...
# 다음뉴스 감성분석 결과 클라우드에서 불러오기(취합)
def daum_news_data_for_sim(s3, bucket_name):
    ...
    object_nm2 = str(formatted_yesterday) + '/daum_news_crawling/' # 폴더 이름
...
#감성분석 결과로 감정비율, 감정지수 계산
def kr_sentiment_for_sim(daum_news):

    daum_news['label'] = daum_news['감성분석'].apply(lambda x: x[0]['label'])
    daum_news['score'] = daum_news['감성분석'].apply(lambda x: x[0]['score'])
    daum_news.drop(columns=['감성분석'], inplace=True)
    daum_news = daum_news[['검색어', 'label', 'score']]


    # [Ticker, label]로 그룹화하고 각 그룹의 개수, 비율, 그리고 'score' 열의 평균 계산
    df_grouped = daum_news.groupby(['검색어', 'label']).agg({
        'score': ['count', 'mean']
    }).reset_index()

    # 열 이름 재설정
    df_grouped.columns = ['검색어', 'label', 'count', 'score_mean']

    # 각 그룹에서 전체 개수를 기준으로 비율 계산
    total_counts = df_grouped.groupby('검색어')['count'].transform('sum')
    df_grouped['ratio'] = df_grouped['count'] / total_counts

    # Pivot을 사용하여 각 Ticker 별로 레이블에 대한 열을 추가
    df_pivoted = df_grouped.pivot(index='검색어', columns='label', values=['count', 'score_mean', 'ratio'])
    ...
    df.rename(columns={'검색어':'기업'}, inplace=True)

    return df



def kr_sentiment_sim_main():
    s3, bucket_name, _ = s3_setting()
    merged_df = daum_news_data_for_sim(s3, bucket_name)
    df = kr_sentiment_for_sim(merged_df)
