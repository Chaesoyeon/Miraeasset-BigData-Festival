...
# 미국 차트분석 데이터 클라우드에서 불러오기
def us_keyword_chart_data(s3, bucket_name, ticker):

    xlsx_filename = f'{ticker}_차트분석.xlsx'
    object_name = str(datetime.datetime.now().strftime('%Y%m%d')) + '/stock_chart_usa/' + str(xlsx_filename)
...
#미국뉴스 요약감성키워드 데이터 클라우드에서 불러오기
def keyword_us_pipe_data(s3, bucket_name):

    #미국뉴스_요약감성키워드 데이터 불러오기
    pickle_filename = 'usa_news_pipe.pickle'
    pk_object_name = str(datetime.datetime.now().strftime('%Y%m%d')) + '/usa_news_crawling/' + str(pickle_filename)
...
#4구간에 해당되는 뉴스의 키워드, 등장 시점, 요약 텍스트, 기사 링크 추가
def keyword_chart_make_df(date, df):
...
#클라우드에 업로드
def keyword_us_upload(ticker, data, s3, bucket_name):
    ...
    # xlsx 파일 클라우드 업로드
    object_name = str(datetime.datetime.now().strftime('%Y%m%d')) + '/keyword_chart_usa/' + str(xlsx_filename)
    s3.upload_fileobj(xlsx_buffer, bucket_name, object_name)
    print(f"Uploaded {xlsx_filename} to S3")


# main 실행 함수
def keyword_chart_us_main():

    s3, bucket_name, _ = s3_setting()
    us_company_list = get_uscomp_list()
    # 클라우드 폴더 생성
    object_name = str(datetime.datetime.now().strftime('%Y%m%d')) + '/keyword_chart_usa/'
    s3.put_object(Bucket=bucket_name, Key=object_name)

    df = keyword_us_pipe_data(s3, bucket_name)

    for index, row in us_company_list.iterrows():
        # company_name = row['company2']
        ticker = row['Ticker']

        date = us_keyword_chart_data(s3, bucket_name, ticker)

        df = df[df['Ticker']==ticker]
        date = keyword_chart_make_df(date, df)

        # 주가 데이터 수집 및 파일로 저장
        keyword_us_upload(ticker, date, s3, bucket_name)